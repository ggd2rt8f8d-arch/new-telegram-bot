...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        """)
        # Инициализируем счётчик запросов
...
async def process_edit(message: Message, state: FSMContext):
    ...
    # Обработка обложки (фото или ссылка)
    if field == "poster":
        if message.photo:
            value = message.photo[-1].file_id  # берём самое качественное фото
        else:
...
async def add_year(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Отправь <b>обложку</b>:\n"
        "• Можно просто прислать фотографию\n"
        "• Или прямую ссылку на картинку",
        parse_mode="HTML"
    )
...
async def add_poster(message: Message, state: FSMContext):
    if message.photo:
        ...
        return await message.answer("Отправь фото или ссылку на обложку:")
...
async def add_description(message: Message, state: FSMContext):
    ...
    await message.answer("Оценка <b>IMDb</b> (например 8.7):", parse_mode="HTML")
...
async def add_rating(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Фильм <b>{data['title']}</b> успешно добавлен!", parse_mode="HTML")
    await state.clear()
...
async def cb_remove_admin(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text("Введи Telegram ID админа, которого нужно снять:")
    await state.set_state(AddAdmin.waiting_id)
...
async def process_admin(message: Message, state: FSMContext):
    ...
    if data.get("action") == "add":
        ...
        await message.answer(f"✅ <code>{uid}</code> больше не админ", parse_mode="HTML")
    await state.clear()
